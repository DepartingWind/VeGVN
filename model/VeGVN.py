...
class Net(torch.nn.Module):
    ...
    def Fusion(self, data):
        ...
        clip_fused = torch.cat((clip_gcn, clip_img_feat), dim=1)
        if clip_fused.size(-1) != (out_feats + hid_feats):
            clip_fused_layer = nn.Linear(clip_fused.size(-1), out_feats + hid_feats).to(device1)
            clip_fused = clip_fused_layer(clip_fused)
        clip_fused = self.clip_shared_linear(clip_fused)
